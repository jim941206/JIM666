...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    // 尋找 "第一單元講義" 的連結
    const lectureLink = Array.from(document.querySelectorAll('.side-menu a')).find(
        a => a.textContent === '第一單元講義'
    );

    if (lectureLink) {
        lectureLink.addEventListener('click', (e) => {
            e.preventDefault(); // 防止連結跳轉
            contentIframe.src = 'https://hackmd.io/@zyy931120/rJnWfSRogl';
            iframeOverlay.style.display = 'flex'; // 顯示 iframe 浮動視窗
        });
    }

    // 關閉按鈕事件
    closeBtn.addEventListener('click', () => {
        iframeOverlay.style.display = 'none'; // 隱藏 iframe 浮動視窗
        contentIframe.src = ''; // 清空 iframe 來源，停止載入
    });
});
